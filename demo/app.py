...
#     Webcam Stream (powered by WebRTC ⚡️)
#     </h1>
#     """)
#     with gr.Column(elem_classes=["my-column"]):
#         with gr.Group(elem_classes=["my-group"]):
#             video = WebRTC(label="Stream")
...
def detection(image, conf_threshold=0.3):
    ...
    return cv2.resize(new_image, (1280, 720))
...
stream = Stream(
    ...
    mode="send-receive",
    additional_inputs=[
        gr.Slider(minimum=0, maximum=1, step=0.01, value=0.3, label="Confidence Threshold"),
    ],
    additional_outputs=[
        gr.Markdown(
            """
    ...
    additional_outputs_handler=lambda _: "test",
    rtc_configuration=get_twilio_turn_credentials() if get_space() else None,
    concurrency_limit=2 if get_space() else None,
    ui_args={"title": "<h1 style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; text-align: center; font-size: 3rem; font-weight: 700; letter-spacing: -0.02em; margin: 2rem 0; font-family: system-ui, -apple-system, sans-serif;'>Real-Time Video Processing</h1><p style='text-align: center; color: #64748b; font-size: 1.1rem; margin-top: -1rem; font-weight: 400;'>Powered by FastRTC</p>"}
    )
